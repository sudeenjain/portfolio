...
async function handleFileSelect(inputElement, previewBoxId, pathHiddenId) {
    ...
    try {
        ...
        if (previewBox) {
            ...
            const pdfIcon = previewBox.querySelector('.preview-item-pdf-icon');
            ...
            if (thumb && file.type.startsWith('image/')) {
                ...
                if (pdfIcon && (file.type === 'application/pdf' || file.name.toLowerCase().endsWith('.pdf'))) {
    ...
    try {
        ...
        if (table === 'documents') {
            try {
                ...
                console.warn("Supabase documents table failed, falling back to localStorage", supabaseErr);
                data = getLocalDocuments();
            }
        } else {
            if (!window.supabaseClient) throw new Error("Offline mode");
            let query = window.supabaseClient.from(table).select('*');
...
function renderTableRows(table, data) {
    ...
    if (table === 'certificates') {
        ...
        data.forEach(item => {
            ...
            const formatText = isPdf ? 'PDF' : 'Image';
            const iconClass = isPdf ? 'fas fa-file-pdf text-danger' : 'fas fa-file-image text-success';
            const rowIcon = `<i class="${iconClass}" style="font-size: 1.5rem; filter: drop-shadow(0 0 5px rgba(0,243,255,0.2));"></i>`;
            const pubBadge = item.published ? `<span class="badge badge-published">Published</span>` : `<span class="badge badge-unpublished">Draft</span>`;
            
            const resolvedUrl = resolveMediaUrl(item.file_url);
            const previewAction = isPdf 
                ? `window.open('${resolvedUrl}', '_blank')` 
                : `openImageLightbox('${resolvedUrl}', '${escapeHTML(item.name)}')`;

            rowsHTML += `
                <tr>
                    <td><div style="width: 40px; text-align: center;">${rowIcon}</div></td>
                    <td><strong style="color:white;">${escapeHTML(item.name)}</strong></td>
                    <td><span style="color:var(--primary); font-size:0.8rem; font-weight: 600;">${formatText}</span></td>
                    <td>${pubBadge}</td>
                    <td class="actions-cell">
                        <button class="btn-icon edit" onclick="${previewAction}" title="Preview Document"><i class="fas fa-eye"></i></button>
                        <a href="${resolvedUrl}" download class="btn-icon edit" style="display: inline-flex; align-items: center; justify-content: center; text-decoration: none;" title="Download Document"><i class="fas fa-download"></i></a>
                        <button class="btn-icon edit" onclick="editRecord('documents', '${item.id}')" title="Edit Name/File"><i class="fas fa-edit"></i></button>
                        <button class="btn-icon delete" onclick="triggerDelete('documents', '${item.id}')" title="Delete Document"><i class="fas fa-trash"></i></button>
                    </td>
                </tr>
            `;
        });
...
function setupSearchFilters() {
    const list = [
        ...
        { id: 'docs-search', table: 'documents', fields: ['name', 'file_url'] }
    ];
...
function openAddModal(table) {
    ...
    try {
        ...
        if (table === 'certificates') {
            ...
            clearFileSelection('doc-file', 'doc-preview', 'doc-path');
...
function editRecord(table, id) {
    ...
    try {
        ...
        if (table === 'certificates') {
            ...
            document.getElementById('doc-name').value = item.name || '';
            document.getElementById('doc-published').checked = !!item.published;

            setExistingFilePreview('doc-preview', 'doc-path', item.file_url);
        }
...
function setupFormListeners() {
    const list = ['certificates', 'badges', 'internships', 'projects', 'achievements', 'skills', 'documents'];
            ...
            try {
                ...
                if (table === 'certificates') {
                    ...
                    payload = {
                        ...
                        file_url: document.getElementById('doc-path').value || null,
                        published: document.getElementById('doc-published').checked
                    };
                }
    ...
    if (logoutBtn) {
        logoutBtn.addEventListener('click', async (e) => {
            ...
            try {
                ...
                // Force clear Supabase session keys from localStorage
                ...
                // Fallback redirect even if API signOut fails but we cleared storage
                window.location.href = '../index.html';
            }
        });
    }
...
// -------------------------------------------------------------
// 11. LOCAL DOCUMENTS STATE REPOSITORY (Supabase Fallback)
// -------------------------------------------------------------
function getLocalDocuments() {
    let docs = localStorage.getItem('admin-documents');
    if (!docs) {
        const defaultDocs = [
            { id: 'doc-1', name: '10th Marksheet', file_url: 'docs/10th.pdf', published: true, display_order: 1, created_at: new Date().toISOString() },
            { id: 'doc-2', name: '12th Marksheet', file_url: 'docs/12th.pdf', published: true, display_order: 2, created_at: new Date().toISOString() },
            { id: 'doc-3', name: '6th Sem Marksheet', file_url: 'docs/6th Sem.pdf', published: true, display_order: 3, created_at: new Date().toISOString() },
            { id: 'doc-4', name: 'APAAR Card', file_url: 'docs/APAAR.pdf', published: true, display_order: 4, created_at: new Date().toISOString() },
            { id: 'doc-5', name: 'Aadhaar Card', file_url: 'docs/Adhar Card (2).pdf', published: true, display_order: 5, created_at: new Date().toISOString() },
            { id: 'doc-6', name: 'Driving License', file_url: 'docs/Driving License.pdf', published: true, display_order: 6, created_at: new Date().toISOString() },
            { id: 'doc-7', name: 'PAN Card', file_url: 'docs/Pan Card.pdf', published: true, display_order: 7, created_at: new Date().toISOString() },
            { id: 'doc-8', name: 'Ration Card', file_url: 'docs/Ration Card.pdf', published: true, display_order: 8, created_at: new Date().toISOString() },
            { id: 'doc-9', name: 'Internshala Resume', file_url: 'docs/Sudeen JainH R_InternshalaResume.pdf', published: true, display_order: 9, created_at: new Date().toISOString() },
            { id: 'doc-10', name: 'Canara Bank Statement', file_url: 'docs/canara bank(statement).pdf', published: true, display_order: 10, created_at: new Date().toISOString() },
            { id: 'doc-11', name: 'Caste Certificate', file_url: 'docs/caste certificate.pdf', published: true, display_order: 11, created_at: new Date().toISOString() },
            { id: 'doc-12', name: 'Income Certificate', file_url: 'docs/income certificate.pdf', published: true, display_order: 12, created_at: new Date().toISOString() },
            { id: 'doc-13', name: 'Profile Photo', file_url: 'docs/profile.jpg', published: true, display_order: 13, created_at: new Date().toISOString() }
        ];
        localStorage.setItem('admin-documents', JSON.stringify(defaultDocs));
        docs = JSON.stringify(defaultDocs);
    }
...
function saveLocalDocument(doc) {
    ...
    if (doc.id) {
        ...
        doc.display_order = docs.length + 1;
        docs.push(doc);
    }
    localStorage.setItem('admin-documents', JSON.stringify(docs));
}

function deleteLocalDocument(id) {
    const docs = getLocalDocuments();
    const filtered = docs.filter(x => x.id !== id);
    localStorage.setItem('admin-documents', JSON.stringify(filtered));
}
